  ...
  return (
    <div className="bg-white rounded-[2.5rem] p-5 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.05)] border border-gray-100 relative mb-5 transition-transform hover:scale-[1.01]">
      ...
      <div className="absolute top-4 right-5 flex items-center gap-2">
        ...
        <div className={`w-3.5 h-3.5 rounded-full ${getLevelColor(glyph.level)} shadow-sm shadow-black/10`}></div>
      </div>

      <div className="flex gap-4 items-center">
        {/* Para cargar las imagenes del glifo */}
        <div className="w-24 h-24 bg-gray-50 rounded-2xl flex items-center justify-center border-2 border-gray-100 overflow-hidden">
          {glyph.image ? (
            <img src={glyph.image} alt={glyph.name} className="w-20 h-20 object-contain" />
          ) : (
            <div className="text-gray-200 text-3xl font-black">?</div>
          )}
        </div>

        <div className="flex flex-col gap-1 grow">
          <div className="flex items-baseline gap-1.5">
            <span className="font-bold text-maya-dark">{glyph.name}</span>
            <span className="text-[10px] font-black text-maya-gold uppercase tracking-tighter">(maya)</span>
          </div>
          <div className="flex items-baseline gap-1.5">
            <span className="font-bold text-maya-dark">{glyph.meaning}</span>
            <span className="text-[10px] font-black text-maya-gold uppercase tracking-tighter">(español)</span>
          </div>
